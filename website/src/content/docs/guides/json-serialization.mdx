...
description: Automatically localize ILocalizable properties during JSON serialization with three configurable modes
sidebar:
  order: 3
---
...
<Tabs>
  ...
  <TabItem label="Exhaustive">
    ...
    Exhaustive mode calls `ILocalizationFormatter.Format<T>()` on every property, which involves DI service resolution and reflection. Use only for debugging or testing — not recommended for production workloads.
    </Aside>
  </TabItem>
</Tabs>
...
<Aside type="tip" title="ASP.NET Core culture handling">
...
<LinkCard title="Localizable Types" description="Define types that participate in localization" href="/Localization/concepts/localizable-types/" />
<LinkCard title="ASP.NET Core Integration" description="Automatic JSON localization in the ASP.NET Core pipeline" href="/Localization/guides/aspnetcore-integration/" />
