...
See the [Custom Providers guide](./custom-providers) for implementation details on both provider approaches
...
| Trailing commas | last item in object | `"key": "value",` |
...
See [Additive Configuration](./additive-configuration) for layering semantics.
...
See the [ASP.NET Core Integration guide](./aspnetcore-integration) for full details on middleware configuration.
